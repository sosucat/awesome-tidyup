...
A small helper to extract file metadata from an exported Canvas HTML file (`CS-6150-O01.html`) and write the results to a CSV.

Contents
 - `src/awesome_tidyup/extract_files.py` — extractor script
 - `CS-6150-O01.html` — input HTML (exported file)
 - `files.csv` — output generated by the script
...
2. Run the extractor (reads `CS-6150-O01.html` and writes `files.csv`):

    ```
    pixi run python -m src.awesome_tidyup.extract_files -i CS-6150-O01.html -o files.csv
    ```
